...
import type { RawUsageMeters } from "./usage.schema";
...
export function buildDashboardUrl(accountId: string, bucketName: string) {
...
async function fetchGraphQlMeters(
  accountId: string,
  accessToken: string,
...
async function fetchCycleToDateCostUsd(
  accountId: string,
  accessToken: string,
...
export async function getAccountUsage({
  ...
  const { data: meters, error: metersError } = await fetchGraphQlMeters(
    accountId,
    accessToken,
  );
  ...
  const { data: cycleToDateUsd } = await fetchCycleToDateCostUsd(
    accountId,
    accessToken,
  );
